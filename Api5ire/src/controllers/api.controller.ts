...
export class ApiController {
    ...
    try {
      ...
      if (azChecking.isAzDomain) {
        ...
        if (!azDomainName) {
          ...
          return this.response.send({
            status: STATUS.FAILED,
            message: MESSAGE.NO_DOMAIN_NAME,
          });
        }
      } else {
  ...
  async getBidsByBidderAddress(
    @requestBody(RequestGetBidsByBidderAddressBody)
    req: ReqGetBidsByBidderAddressType,
  ): Promise<ResponseBody | Response> {
    try {
      if (!req) {
        // @ts-ignore
      ...
      if (!offset) offset = 0;
      const order =
        req?.sort && req?.sort == 1 ? 'bid_date ASC' : 'bid_date DESC';
      if (!bidder) {
        // @ts-ignore
        return this.response.send({
          status: STATUS.FAILED,
    ...
    try {
      ...
      if (input_data) {
        if (is1024 && input_data.location1024 != '') {
          ...
          return this.response.send({
            status: STATUS.FAILED,
            message: MESSAGE.INPUT_ALREADY_EXIST,
          });
        }
    ...
    try {
      ...
      if (images.length) {
        ...
        for (let i = 0; i < images.length; i++) {
          ...
          if (input_data) {
            if (is1024 && input_data.location1024 != '') continue;
            if (is1440 && input_data.location1440 != '') continue;
            if (is1920 && input_data.location1920 != '') continue;
            if ((is1024 || is1440 || is1920) && input_data.location100 != '') {
              is500 = false;
              is100 = false;
            }
          ...
          if (!queue_data) {
            ...
            if (imageType == 'collection' || imageType == 'launchpad') {
              if (metadata?.collectionAddress && global_vars.caller) {
                ...
                if (collection) {
                  ...
                  if (attributes[0]) {
                    ...
                    console.log(
                      'Collection missing ' + metadata.type + ' type',
                    );
                  }
                } else {
              ...
              if (metadata.walletAddress && global_vars.caller) {
                let attributes = await profile_calls.getAttributes(
                  global_vars.caller,
              ...
              if (imageType == 'nft') {
                if (
                  ...
                  let attributes =
                    await nft721_psp34_standard_calls.getAttributes(
                      nft_contract,
                      global_vars.caller,
  ...
  async getCollectionContract(
    @requestBody(RequestGetCollectionContractBody)
    req: ReqGetCollectionContractType,
  ): Promise<ResponseBody | Response> {
    try {
      // @ts-ignore
  ...
  async getFeaturedCollections(): Promise<ResponseBody | Response> {
    try {
      ...
      if (process.env.LIST_FEATURED_COLLECTIONS) {
        const listFeaturedCollections =
          process.env.LIST_FEATURED_COLLECTIONS.split(',');
  ...
  async updateCollectionEmail(
    @requestBody(RequestUpdateCollectionEmailBody)
    req: ReqUpdateCollectionEmailType,
  ): Promise<ResponseBody | Response> {
    try {
      if (!req) {
        // @ts-ignore
  ...
  async getCollectionsByVolume(
    @requestBody(RequestGetCollectionsByVolumeBody)
    req: ReqGetCollectionsByVolumeType,
  ): Promise<ResponseBody | Response> {
    try {
      if (!req) {
        // @ts-ignore
  ...
  async getCollectionsByOwner(
    @requestBody(RequestGetCollectionsByOwnerBody)
    req: ReqGetCollectionsByOwnerType,
  ): Promise<ResponseBody | Response> {
    try {
      if (!req) {
        // @ts-ignore
  ...
  async countCollectionsByOwner(
    @requestBody(RequestCountCollectionsByOwnerBody)
    req: ReqCountCollectionsByOwnerType,
  ): Promise<ResponseBody | Response> {
    try {
      if (!req) {
        // @ts-ignore
  ...
  async getCollectionByAddress(
    @requestBody(RequestGetCollectionByAddressBody)
    req: ReqGetCollectionByAddressType,
  ): Promise<ResponseBody | Response> {
    try {
  ...
  async getNFTsByOwnerAndCollection(
    @requestBody(RequestGetNFTsByOwnerAndCollectionBody)
    req: ReqGetNFTsByOwnerAndCollectionType,
  ): Promise<ResponseBody | Response> {
    try {
      if (!req) {
        // @ts-ignore
  ...
  async getNFTsByCollectionAddress(
    @requestBody(RequestGetNFTsByCollectionAddressBody)
    req: ReqGetNFTsByCollectionAddressType,
  ): Promise<ResponseBody | Response> {
    try {
      if (!req) {
        // @ts-ignore
  ...
  async getNewListEvents(
    ...
    try {
      ...
      const order =
        req?.sort && req?.sort == 1 ? 'blockNumber ASC' : 'blockNumber DESC';
      if (collection_address) {
  ...
  async getOwnershipHistory(
    @requestBody(RequestGetOwnershipHistoryBody)
    req: ReqGetOwnershipHistoryType,
  ): Promise<ResponseBody | Response> {
    try {
      if (!req) {
        // @ts-ignore
  ...
  async searchNFTOfCollectionByTraits(
    @requestBody(RequestSearchNFTOfCollectionByTraitsBody)
    req: ReqSearchNFTOfCollectionByTraitsType,
  ): Promise<ResponseBody | Response> {
    try {
      // console.log(req);
      ...
      let params: TraitFilters | undefined = req.traitFilters
        ? JSON.parse(req.traitFilters)
        : undefined;
      ...
      if (params?.keyword) {
        paramTmp = {
          ...paramTmp,
          nftName: {like: `${params.keyword}`, options: 'i'},
        };
      }
  ...
  async getAddRewardHistory(
    @requestBody(RequestGetAddRewardHistoryBody)
    req: ReqGetAddRewardHistoryType,
  ): Promise<ResponseBody | Response> {
    try {
      if (!req) {
        // @ts-ignore
  ...
  async getClaimRewardHistory(
    @requestBody(RequestGetClaimRewardHistoryBody)
    req: ReqGetClaimRewardHistoryType,
  ): Promise<ResponseBody | Response> {
    try {
      if (!req) {
        // @ts-ignore
  ...
  async findNfts(@param.filter(nfts) filter?: Filter<nfts>): Promise<nfts[]> {
  ...
  async createBlackList(
    ...
    try {
      ...
      const isActive: boolean =
        req?.isActive !== undefined ? req.isActive : true;
      const userName = req?.userName;
      const password = req?.password;
      if (
        userName !== process.env.BLACK_LIST_USER_NAME ||
        password !== process.env.BLACK_LIST_PASSWORD
      ) {
        // @ts-ignore
        return this.response.send({
          status: STATUS.FAILED,
  ...
  async findBlackListById(
    ...
    @param.filter(BlackList, {exclude: 'where'})
    filter?: FilterExcludingWhere<BlackList>,
  ): Promise<BlackList> {
        ...
        if (currentNftQueue && currentNftQueue.length > 0) {
          for (const nftQueue of currentNftQueue) {
            if (nftQueue.type === 'update') {
              const currentNftQueueAll =
                await this.nftQueueScanAllSchemaRepository.findOne({
                  where: {
              ...
              if (currentNftQueueAll) {
                console.log(
                  `deleteById: ${nftQueue.nftContractAddress} - tokenID: ${nftQueue.tokenID}`,
                );
                await this.nftQueueSchemaRepository.deleteById(nftQueue._id);
              } else {
                console.log(
                  `create: ${nftQueue.nftContractAddress} - tokenID: ${nftQueue.tokenID}`,
                );
  ...
  async getWithdrawEvent(
    ...
    const eventCount = await this.withdrawEventSchemaRepository.count(
      filter?.where,
    );
  ...
  @response(200, {
    ...
    content: {
      'application/json': {
        schema: {
          type: 'array',
          items: getModelSchemaRef(launchpadmintingevents, {
  ...
  async getLaunchpadMintingEvent(
    @param.filter(launchpadmintingevents)
  ...
  async getLaunchpadMintingEventV1(
    @requestBody(RequestGetLaunchpadMintingEventBody)
    req: ReqGetLaunchpadMintingEventType,
  ): Promise<ResponseBody | Response> {
    if (!req || !req.nftContractAddress) {
      // @ts-ignore
      return this.response.send({
  ...
  async checkingImagesAndJson(
    @requestBody(RequestCheckingImagesAndJsonBody)
    req: ReqCheckingImagesAndJsonType,
  ): Promise<ResponseBody | Response> {
    try {
      if (!req || !req.userName || !req.password) {
        // @ts-ignore
      ...
      try {
        ...
        // @ts-ignore
        const {result, output} =
          await launchpad_psp34_nft_standard_contract.query[
        ...
        if (currentSupply) {
          for (let tokenID = 1; tokenID <= currentSupply; tokenID++) {
            console.log(
              `Checking tokenID: ${tokenID} of tokenUri ${tokenUri} from currentSupply ${currentSupply} NFTs`,
            );
            const input = `${tokenUri.replace(
              'ipfs://',
              '/ipfs/',
            )}${tokenID}.json`;
            let input_data = await this.jsonSchemaRepository.findOne({
            ...
            if (!input_data) {
              ...
              let imageIpfsLinkI = offChainData?.image.replace(
                'ipfs://',
                '/ipfs/',
              );
              if (imageIpfsLinkI) {
        ...
        if (allCollection) {
          for (const collection of allCollection) {
            if (collection.isActive) {
              ...
              console.log(
                `${collection.nftContractAddress}: ${nftsForSale.length} NFTs`,
              );
    ...
    try {
      ...
      try {
        ...
        if (!foundCollection) {
          ...
          try {
            await this.collectionsSchemaRepository.updateById(
              foundCollection._id,
              {
  ...
  async adGetListMinter(
    ...
    try {
      ...
      try {
        ...
        for (const collection of collections) {
          ...
          if (collection) {
            try {
              ...
              const mintedData =
                await this.launchpadMintingEventSchemaRepository.find({
                  where: filterMode,
                });
              for (const item of mintedData) {
                if (
                  item?.minter &&
                  item?.mintingFee != undefined &&
                  item?.mintAmount
                ) {
                  mintingFeeData.set(
                    item.minter,
                    (mintingFeeData.get(item.minter) || 0) + item.mintingFee,
                  );
                  mintingAmountData.set(
                    item.minter,
                    (mintingAmountData.get(item.minter) || 0) + item.mintAmount,
                  );
                    ...
                    if (whitelistData.phaseId == 1) {
                      ...
                      if (userData) {
                        for (const user of userData) {
                          if (
                            user.whitelistAmount &&
                            user.whitelistAmount > 0 &&
                            user.claimedAmount != undefined &&
              ...
              data.push({
                ...
                listAccMintedInWhitelistAddress: listAccMintedInWhitelist.map(
                  item => item.address,
                ),
                listAccUnMintedInWhitelistAddress:
                  listAccUnMintedInWhitelist.map(item => item.address),
              });
            } catch (e) {
              return this.response.send({
                status: STATUS.FAILED,
        ...
        let sortedMintingFeeData = new Map(
          [...mintingFeeData.entries()].sort((a, b) => a[1] - b[1]),
        );
        let sortedMintingAmountData = new Map(
          [...mintingAmountData.entries()].sort((a, b) => a[1] - b[1]),
        );
        return this.response.send({
          status: STATUS.OK,
          message: MESSAGE.SUCCESS,
  ...
  async getBidByCollection(
    ...
    try {
      ...
      for (const nft of nftLive) {
        ...
        console.log(
          `Total bids of ${nft.tokenID} of collection ${nft.nftContractAddress}: ${bids.length}`,
        );
        for (const bid of bids) {
          console.log(
            `Sum of ${bid.bid_value} from ${bid.tokenID} of collection ${bid.nftContractAddress}`,
          );
          if (bid.bid_value) {
  ...
  async getUserBuySellEvent(
    ...
    const purchaseEventData = await this.purchaseEventSchemaRepository.find({
      ...filter,
      offset: 0,
      limit: 9999999,
    });

    const bidWinEventData = await this.bidWinEventSchemaRepository.find({
      ...filter,
      offset: 0,
      limit: 9999999,
    });
    ...
    const eventDataType = eventData?.seller
      ? 'seller'
      : eventData?.buyer
      ? 'buyer'
      : 'n/a';
