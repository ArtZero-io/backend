...
export class ApiController {
  ...
  async getBidWinEvents(
    ...
    try {
      ...
      const order =
        req?.sort && req?.sort == 1 ? 'blockNumber DESC' : 'blockNumber ASC';
      if (collection_address) {
  ...
  async triggerRewards(
    ...
    try {
      ...
      const password = req?.password;
      if (
        userName !== process.env.TRIGGER_REWARDS_USER_NAME ||
        password !== process.env.TRIGGER_REWARDS_PASSWORD
      ) {
        // @ts-ignore
        return this.response.send({
          status: STATUS.FAILED,
  ...
  async getLaunchpadMintingEvent(
    ...
    const eventCount = await this.luanchpadMintingEventSchemaRepository.count(
      filter?.where,
    );
    // @ts-ignore
    return this.response.send({
